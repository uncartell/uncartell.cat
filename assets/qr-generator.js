(()=>{
  ...
  $('[data-save-project]').addEventListener('click',()=>{if(!canPremium){toast('Premium');return}if(!generate({quiet:true}))return;sync();const list=projects().filter(project=>project.id!==state.id);const saved={...state,savedAt:new Date().toISOString()};list.unshift(saved);saveProjects(list);$('[data-save-status]').textContent=`${T.savedAt} · ${new Intl.DateTimeFormat(lang,{dateStyle:'short',timeStyle:'short'}).format(new Date(saved.savedAt))}`;toast(T.saved)});
  const renderProjects=()=>{const list=projects();$('[data-project-list]').innerHTML=list.length?list.map(project=>`<article class="qr-project-card"><div><strong>${escapeHtml(project.name)}</strong><small>${escapeHtml(project.url)}</small></div><div><button type="button" data-open="${project.id}">${T.open}</button><button type="button" data-delete="${project.id}">${T.remove}</button></div></article>`).join(''):`<p class="qr-project-empty">${T.none}</p>`;
    ...
    $$('[data-delete]').forEach(button=>button.addEventListener('click',()=>{saveProjects(projects().filter(item=>item.id!==button.dataset.delete));renderProjects();toast(T.deleted)}));
  };
  $$('[data-open-projects]').forEach(button=>button.addEventListener('click',()=>{if(!canPremium){toast('Premium');return}renderProjects();$('[data-projects-modal]').hidden=false}));
  ...
  $('[data-download-svg]').addEventListener('click',()=>{downloadBlob(svgBlob(state.svg),`${safeName(state.name)}.svg`);consumeDownload();toast(T.downloaded)});
  $('[data-download-png]').addEventListener('click',async()=>{downloadBlob(await svgToPng(state.svg),`${safeName(state.name)}.png`);consumeDownload();toast(T.downloaded)});
