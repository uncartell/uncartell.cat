(()=>{
  ...
  const initToolOnboarding=()=>{
    ...
    const render=()=>{const currentSlide=slides[step],last=step===slides.length-1;box.innerHTML=`<div class="u-onboarding-media"><img src="${currentSlide.image}" alt=""><span>${step+1} / ${slides.length}</span></div><button class="u-onboarding-close" type="button" aria-label="${lang==='ca'?'Tanca':'Cerrar'}">×</button><div class="u-onboarding-copy"><strong>${currentSlide.title}</strong><p>${currentSlide.copy}</p><div class="u-onboarding-dots" aria-hidden="true">${slides.map((_,index)=>`<i${index===step?' class="is-active"':''}></i>`).join('')}</div><div class="u-onboarding-actions"><button class="u-onboarding-skip" type="button">${lang==='ca'?'Omet':'Omitir'}</button><button class="u-onboarding-next" type="button">${last?(lang==='ca'?'Comença':'Empezar'):(lang==='ca'?'Següent':'Siguiente')}</button></div></div>`;
