(()=>{
  ...
  if(!document.querySelector('link[data-ui-shape-system]')){
    ...
    shapeStyles.href='/assets/ui-shape-system.css?v=20260822-quota1';
  ...
  const planRank={basic:0,free:0,premium:1,ultra:2};
  const normalizePlan=plan=>Object.prototype.hasOwnProperty.call(planRank,String(plan||'').toLowerCase())?(String(plan).toLowerCase()==='free'?'basic':String(plan).toLowerCase()):'basic';
  function getPlan(){return normalizePlan(localStorage.getItem(planKey))}
  function hasPlan(required){return planRank[getPlan()]>=planRank[normalizePlan(required)]}
  function setPlan(value){const plan=normalizePlan(value);localStorage.setItem(planKey,plan);document.documentElement.dataset.plan=plan;renderQuota();window.dispatchEvent(new CustomEvent('uncartell:plan',{detail:{plan}}))}
