(()=>{
  ...
  if(brief){
    ...
    modal.querySelector('form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,button=form.querySelector('[type="submit"]');button.disabled=true;button.textContent=platform.words.formSending;try{await platform.submitMailboxForm({type:'studio',form,fields:{tool:ca?'Carta o menú':'Carta o menú'}});form.innerHTML=`<div class="custom-brief-success"><strong>${ca?'Enviat!':'¡Enviado!'}</strong><p>${ca?'Gràcies. Et respondrem tan aviat com puguem.':'Gracias. Te responderemos lo antes posible.'}</p></div>`}catch(error){button.disabled=false;button.textContent=ca?'Envia la petició':'Envía la solicitud';let out=form.querySelector('.custom-brief-error');if(!out){out=document.createElement('p');out.className='custom-brief-error';form.appendChild(out)}out.textContent=error.message||platform.words.formError}}
  }
})();
