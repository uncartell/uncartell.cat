  ...
  function renderFormats() {
    $("#formatGrid").innerHTML = L.formats.map(format => `
      <article class="format-card${format.id === "mobile-interactive" ? " format-featured" : ""}" data-format="${format.id}">
        ...
        <span class="format-actions"><button type="button" data-format-action="demo"><svg viewBox="0 0 24 24"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="2.5"/></svg>${escapeHtml(L.demo)}</button><a href="?format=${encodeURIComponent(format.id)}" data-format-action="personalize"><svg viewBox="0 0 24 24"><path d="M12 3a9 9 0 1 0 0 18h1.2a1.8 1.8 0 0 0 0-3.6H12a1.5 1.5 0 0 1 0-3h2.5A6.5 6.5 0 0 0 21 8c0-2.8-4-5-9-5Z"/><circle cx="7.5" cy="10" r=".7"/><circle cx="10" cy="7" r=".7"/><circle cx="14" cy="7" r=".7"/></svg>${escapeHtml(L.personalizeFormat)}</a></span>
      </article>`).join("");
