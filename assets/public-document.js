(()=>{
  ...
  const allergenIcon=(key,withLabel=false)=>{const label=allergenLabels[lang][key]||key;return `<button type="button" class="u-public-allergen${withLabel?' has-visible-label':''}" title="${escape(label)}" aria-label="${escape(label)}" aria-expanded="false"><svg viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet" aria-hidden="true">${allergenPaths[key]||'<circle cx="12" cy="12" r="7"/>'}</svg>${withLabel?`<span class="u-public-allergen-name">${escape(label)}</span>`:`<span class="u-public-allergen-tooltip" role="tooltip">${escape(label)}</span>`}</button>`};
