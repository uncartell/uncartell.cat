(()=>{
  ...
  const allergenIcon=(key,withLabel=false)=>{const label=allergenLabels[lang][key]||key;return `<span class="u-public-allergen" title="${escape(label)}" aria-label="${escape(label)}"><svg viewBox="0 0 24 24" aria-hidden="true">${allergenPaths[key]||'<circle cx="12" cy="12" r="7"/>'}</svg>${withLabel?`<span>${escape(label)}</span>`:''}</span>`};
