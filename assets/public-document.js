(()=>{
  ...
  const renderStatus=status=>{const copy=lang==='es'?{not_found:['Página no encontrada','Esta página no se ha encontrado.'],blocked:['Página temporalmente bloqueada','Por razones de seguridad o por un posible incumplimiento de nuestros términos y condiciones, hemos bloqueado temporalmente esta página. Contacta con hola@uncartel.es.'],deleted:['Esta página ya no está disponible','Esta página ha sido eliminada por su administrador o por el equipo de uncartel.es. Si no lo habéis solicitado, contactad con hola@uncartel.es.'],expired:['Esta página ya no está disponible','Esta página ha caducado. Si sois sus administradores, contactad con hola@uncartel.es.']}:{not_found:['Pàgina no trobada','Aquesta pàgina no s’ha trobat.'],blocked:['Pàgina temporalment bloquejada','Per raons de seguretat o per un possible incompliment dels nostres termes i condicions, hem bloquejat temporalment aquesta pàgina. Contacteu amb hola@uncartell.cat.'],deleted:['Aquesta pàgina ja no està disponible','Aquesta pàgina ha estat eliminada pel seu administrador o per l’equip d’uncartell.cat. Si no ho heu sol·licitat vosaltres, poseu-vos en contacte amb hola@uncartell.cat.'],expired:['Aquesta pàgina ja no està disponible','Aquesta pàgina ha caducat. Si en sou els administradors, contacteu amb hola@uncartell.cat.']};const message=copy[status]||copy.not_found;const main=document.querySelector('main');main.className='u-public-document u-public-status';main.innerHTML=`<div class="u-public-shell"><section class="u-public-home"><span class="u-public-kicker">${status==='not_found'?'404':lang==='es'?'AVISO':'AVÍS'}</span><h1>${escape(message[0])}</h1><p>${escape(message[1])}</p><a href="/" class="u-public-status-home">${lang==='es'?'Volver al inicio':'Torna a l’inici'}</a></section></div>`;document.body.classList.add('u-public-mode')};
