(() => {
  ...
  const tr=(ca,es,it)=>L.lang==="ca"?ca:L.lang==="it"?it:es;
  if (!L.blockTypes.some(item => item.id === "large-text")) L.blockTypes.splice(Math.min(1, L.blockTypes.length), 0, { id: "large-text", label: tr("Títol gran","Título grande","Titolo grande"), symbol: "Aa" });
  if (!L.blockTypes.some(item => item.id === "price")) L.blockTypes.splice(Math.min(2, L.blockTypes.length), 0, { id: "price", label: tr("Preu","Precio","Prezzo"), symbol: "€" });
  if (!L.blockTypes.some(item => item.id === "image")) L.blockTypes.push({ id: "image", label: tr("Bloc d’imatge","Bloque de imagen","Blocco immagine"), symbol: "▧" });
  L.contentImage = tr("Imatge","Imagen","Immagine");
  L.chooseContentImage = tr("Afegeix una imatge 16:9","Añade una imagen 16:9","Aggiungi un’immagine 16:9");
  L.contentImageHelp = tr("Disponible amb Ultra en composicions de dues columnes.","Disponible con Ultra en composiciones de dos columnas.","Disponibile con Ultra nelle composizioni a due colonne.");
  ...
  const priceBadgeLabels = L.lang === "ca" ? ["Segell sòlid", "Oval sòlid", "Rectangle sòlid", "Segell de contorn", "Oval de contorn", "Rectangle de contorn"] : L.lang === "it" ? ["Sigillo pieno", "Ovale pieno", "Rettangolo pieno", "Sigillo contorno", "Ovale contorno", "Rettangolo contorno"] : ["Sello sólido", "Óvalo sólido", "Rectángulo sólido", "Sello de contorno", "Óvalo de contorno", "Rectángulo de contorno"];
  ...
  const makeBlock = (type, overrides = {}) => {
    ...
    if (type === "large-text") return { ...base, text: tr("El teu títol","Tu título","Il tuo titolo"), ...overrides };
  ...
  const mobilePages = () => {
    ...
    return [
      ...
      mobileSection("mains", ca ? "Principals" : it ? "Secondi" : "Principales", [
      ...
      mobileSection("desserts", ca ? "Postres" : it ? "Dolci" : "Postres", [
  ...
  function renderPage() {
    ...
    if (page.role === "mobile-home") {
      ...
      menu.innerHTML = `${brandLogoMarkup(page)}${blocksLayout(page)}${page.showAllergenLegend ? allergenLegend() : ""}${brandedFooter(page)}${watermark()}`;
  ...
  function staticPageHtml(page) {
    ...
    else content = `${brandLogoMarkup(page, "cover", true)}${blocksLayout(page, staticBlockHtml)}${page.showAllergenLegend ? allergenLegend() : ""}`;
  ...
  let pickerTemplates = [
    { name: tr("Plantilla genèrica","Plantilla genérica","Modello generico"), detail: tr("Una base neta i totalment editable.","Una base limpia y totalmente editable.","Una base pulita e completamente modificabile."), accent: "#e5372a", blank: true }
  ];
  const visiblePickerTemplates = () => pickerTemplates;
  function pickerPreview(format, index) {
    if (format === "mobile-interactive") return `<span class="template-mobile template-variant-${index}"><span class="template-kicker">RESTAURANT L’OLIVERA</span><strong>${tr("La nostra carta","Nuestra carta","Il nostro menu")}</strong><i>${tr("Entrants","Entrantes","Antipasti")}</i><i>${tr("Principals","Principales","Secondi")}</i><i>${tr("Postres","Postres","Dolci")}</i></span>`;
  ...
  function renderTemplatePicker() {
    ...
    grid.innerHTML = visiblePickerTemplates(selectedPickerFormat).map((template, index) => `<article class="format-card format-option${locked ? " is-premium-locked" : ""}" data-format="${selectedPickerFormat}" data-picker-card="${index}" tabindex="0" aria-label="${tr("Personalitza el disseny","Personaliza el diseño","Personalizza il design")} ${escapeHtml(template.name)}"><span class="format-paper-wrap"><span class="format-paper ${selectedPickerFormat}">${pickerPreview(selectedPickerFormat, index)}</span></span>${locked ? '<span class="format-plan-badge">Premium</span>' : ""}<span class="format-copy"><h2>${escapeHtml(template.name)}</h2><p>${escapeHtml(template.detail)}</p><p class="format-meta">${escapeHtml(definition?.fold || "")}</p></span><span class="format-actions"><button type="button" data-picker-template="${index}">${tr("Personalitza","Personaliza","Personalizza")}</button></span></article>`).join("");
