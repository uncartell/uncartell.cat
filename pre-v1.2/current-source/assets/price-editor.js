(() => {
  ...
  const ca = L.lang === "ca";
  L.defaultProject = ca ? "Tarifes Hanoi SPA" : "Tarifas Hanoi SPA";
  L.blockTypes = L.blockTypes.map(item => ({ ...item, label: item.id === "dish" ? (ca ? "Servei" : "Servicio") : item.id === "dish-image" ? (ca ? "Servei + imatge" : "Servicio + imagen") : item.label }));
  L.imageDish = ca ? "Imatge del servei" : "Imagen del servicio";
  L.chooseDishImage = ca ? "Afegeix una fotografia" : "Añade una fotografía";
  L.removeDishImage = ca ? "Elimina la fotografia" : "Elimina la fotografía";
  L.imageDishHelp = ca ? "La fotografia apareix sobre la informació del servei." : "La fotografía aparece sobre la información del servicio.";
  L.dishImageTooLarge = ca ? "La imatge del servei ha de pesar menys de 4 MB." : "La imagen del servicio debe pesar menos de 4 MB.";
  L.inspector = { ...L.inspector, name: ca ? "Nom del servei" : "Nombre del servicio", restaurant: ca ? "Nom del negoci" : "Nombre del negocio", empty: ca ? "Selecciona un element per editar-lo." : "Selecciona un elemento para editarlo." };
  L.footerTextPlaceholder = ca ? "uncartell.cat o el nom del negoci" : "uncartel.es o el nombre del negocio";
  L.defaults = { ...L.defaults, restaurant: "Hanoi SPA", coverTitle: "Hanoi SPA", coverSubtitle: ca ? "Benestar, cura i pausa" : "Bienestar, cuidado y pausa", section: ca ? "Massatges" : "Masajes", dish: ca ? "Massatge holístic" : "Masaje holístico", description: "50 min", price: "75 €", note: ca ? "Reserva prèvia recomanada." : "Se recomienda reserva previa." };
  const projectStorageKey = `uncartell-price-projects-preview-user-${L.lang}`;
  ...
  const readBrandKit = () => {
    try {
      const raw = JSON.parse(localStorage.getItem(brandKitStorageKey) || "null") || {};
      const kit = { ...defaultBrandKit(), ...raw, version: 2, primary: raw.primary || raw.colorPrimary || raw.primaryColor || "#e5372a", secondary: raw.secondary || raw.colorSecondary || raw.secondaryColor || "#181614", logo: raw.logo || raw.logoData || null, font: raw.font || raw.style || "modern" };
      const isOldTestValue = /(prova\s+de\s+foc|prueba\s+de\s+fuego)/i.test(String(kit.footerText || ""));
      if (!kit.footerTextSet || isOldTestValue) {
        kit.footerText = L.lang === "ca" ? "uncartell.cat" : "uncartel.es";
        kit.footerTextSet = false;
        localStorage.setItem(brandKitStorageKey, JSON.stringify(kit));
      }
      return kit;
    } catch (_) { return defaultBrandKit(); }
  };
  ...
  const starterBlocks = (variant = 0) => variant === 0
    ? [makeBlock("section", { text: ca ? "Massatges" : "Masajes" }), makeBlock("dish", { name: ca ? "Massatge holístic" : "Masaje holístico", description: "50 min", price: "75 €" }), makeBlock("dish", { name: "Slow Relax", description: "30 / 50 / 90 min", price: "50 / 75 / 110 €" }), makeBlock("dish", { name: ca ? "Massatge de teixit profund" : "Masaje de tejido profundo", description: "30 / 50 / 90 min", price: "55 / 85 / 120 €" })]
    : [makeBlock("section", { text: ca ? "Facials i rituals" : "Faciales y rituales" }), makeBlock("dish", { name: "Facial exprés", description: "30 min", price: "45 €" }), makeBlock("dish", { name: ca ? "Neteja facial bàsica" : "Limpieza facial básica", description: "50 min", price: "65 €" }), makeBlock("dish", { name: ca ? "Ritual orgànic sensorial" : "Ritual orgánico sensorial", description: "75 min", price: "120 €" }), makeBlock("dish", { name: ca ? "Ritual oasi asiàtic" : "Ritual oasis asiático", description: "90 min", price: "150 €" })];

  const verticalBlocks = () => [
    makeBlock("section", { text: L.lang === "ca" ? "Vins" : "Vinos" }),
    makeBlock("dish", { name: L.lang === "ca" ? "Blanc de la casa" : "Blanco de la casa", description: L.lang === "ca" ? "Garnatxa blanca · Terra Alta" : "Garnacha blanca · Terra Alta", price: "4,50 €" }),
    makeBlock("dish", { name: L.lang === "ca" ? "Negre del Montsant" : "Tinto del Montsant", description: L.lang === "ca" ? "Garnatxa i carinyena" : "Garnacha y cariñena", price: "22,00 €" }),
    makeBlock("section", { text: L.lang === "ca" ? "Postres" : "Postres" }),
    makeBlock("dish", { name: L.lang === "ca" ? "Crema catalana" : "Crema catalana", price: "6,50 €", allergens: ["egg", "milk"] }),
    makeBlock("dish", { name: L.lang === "ca" ? "Pastís de llimona" : "Tarta de limón", price: "7,00 €", allergens: ["gluten", "egg", "milk"] })
  ];
  ...
  const singleSheetPages = () => [
    { role: "inside", side: "front", showAllergenLegend: true, blocks: starterBlocks(0) },
    { role: "cover", side: "back", restaurant: L.defaults.restaurant, title: L.defaults.coverTitle, subtitle: L.defaults.coverSubtitle, blocks: [] }
  ];
  ...
  const mobilePages = () => {
    return [
      ...
      mobileSection("massages", ca ? "Massatges" : "Masajes", [
        { name: ca ? "Massatge holístic" : "Masaje holístico", description: "50 min", price: "75 €", allergens: [] },
        { name: "Slow Relax", description: "30 / 50 / 90 min", price: "50 / 75 / 110 €", allergens: [] },
        { name: ca ? "Massatge de teixit profund" : "Masaje de tejido profundo", description: "30 / 50 / 90 min", price: "55 / 85 / 120 €", allergens: [] }
      ]),
      mobileSection("facials", ca ? "Tractaments facials" : "Tratamientos faciales", [
        { name: "Facial exprés", description: "30 min", price: "45 €", allergens: [] },
        { name: ca ? "Neteja facial bàsica" : "Limpieza facial básica", description: "50 min", price: "65 €", allergens: [] }
      ]),
      mobileSection("rituals", ca ? "Rituals" : "Rituales", [
        { name: ca ? "Ritual orgànic sensorial" : "Ritual orgánico sensorial", description: "75 min", price: "120 €", allergens: [] },
        { name: ca ? "Ritual oasi asiàtic" : "Ritual oasis asiático", description: "90 min", price: "150 €", allergens: [] }
      ]),
    ];
  };
  ...
  const pagesForFormat = format => {
    ...
    if (format === "price-a3-portrait" || format === "price-a3-landscape") return [{ role: "inside", side: "front", blocks: [...starterBlocks(0), ...starterBlocks(1)] }];
    ...
    if (format === "a4-portrait") pages.filter(page => page.role === "inside").forEach(page => { page.blocks = verticalBlocks(); });
    return pages;
  };
  ...
  const initialState = () => {
    ...
    priceHeader: { title: L.defaults.coverTitle, image: "/assets/price-header-abstract.webp?v=2", textColor: "white" },
    brandKit,
    pages: foldedPages()
  });};
  ...
  function openDemo(format) {
    ...
    const dishes = format === "a4-portrait"
      ? [[L.lang === "ca" ? "Vins" : "Vinos", "Blanc de la casa", "Negre del Montsant"], [L.lang === "ca" ? "Postres" : "Postres", "Crema catalana", L.lang === "ca" ? "Pastís de llimona" : "Tarta de limón"]]
      : [[L.lang === "ca" ? "Per començar" : "Para empezar", L.lang === "ca" ? "Croquetes de rostit" : "Croquetas de cocido", L.lang === "ca" ? "Bunyols de bacallà" : "Buñuelos de bacalao"], [L.lang === "ca" ? "Plats principals" : "Platos principales", L.lang === "ca" ? "Arròs de temporada" : "Arroz de temporada", L.lang === "ca" ? "Peix del dia" : "Pescado del día"]];
    ...
    $("#demoPreview").innerHTML = format === "mobile-interactive"
      ...
      : `<div class="demo-paper demo-${format}"><span class="demo-restaurant">RESTAURANT L’OLIVERA</span><strong>La nostra carta</strong><div class="demo-columns">${dishes.map(group => `<div><b>${escapeHtml(group[0])}</b>${group.slice(1).map((dish, i) => `<span>${escapeHtml(dish)} <em>${i ? "18,50 €" : "9,50 €"}</em></span>`).join("")}</div>`).join("")}</div><small>CREAT AMB UNCARTELL.CAT</small></div>`;
  ...
  function openEditor(format) {
    ...
    if (state.format !== format) {
      ...
      state.contentColumns = format === "price-a3-landscape" ? 2 : 1;
    }
    state.format = format;
    state.activePage = 0;
  ...
  function renderAll() {
    state.pages = state.pages.filter(page => page.role !== "mobile-allergens");
    state.pages.forEach(page => { page.showAllergenLegend = false; (page.blocks || []).forEach(block => { block.allergens = []; }); });
    state.activePage = Math.min(state.activePage, Math.max(0, state.pages.length - 1));
    renderPageTabs();
    renderBlockButtons();
    renderStyles();
    renderInspector();
  ...
  function renderPageTabs() {
    const labels = state.format === "mobile-interactive"
      ...
      : state.format.startsWith("price-a3-") ? [L.lang === "ca" ? "Taula de preus" : "Tabla de precios"]
  ...
  function renderStyles() {
    const mobileOnly = state.format === "mobile-interactive";
    $(".column-settings").hidden = mobileOnly;
    if (mobileOnly) state.contentColumns = 1;
    state.priceHeader ||= { title: L.defaults.coverTitle, image: null, textColor: "white" };
    state.priceHeader.title = String(state.priceHeader.title || L.defaults.coverTitle).slice(0, 28);
    $(".price-header-controls").hidden = mobileOnly;
    $("#priceHeaderTitle").value = state.priceHeader.title;
    $("#priceHeaderWhite").classList.toggle("active", state.priceHeader.textColor === "white");
    $("#priceHeaderBlack").classList.toggle("active", state.priceHeader.textColor === "black");
  ...
  function wireMobilePublication(box) {
    ...
    form.addEventListener("submit", async event => {
      ...
      try{
        ...
        box.innerHTML=`<button class="modal-close" type="button" data-close-mobile-publish>×</button><div class="mobile-publish-success"><span class="mobile-publish-success-mark" aria-hidden="true"><svg viewBox="0 0 64 64"><path d="M19 33.5 28 42l18-21"/><path d="M32 5.5A26.5 26.5 0 1 1 5.5 32 26.5 26.5 0 0 1 32 5.5Z"/></svg></span><h2>${L.lang==='es'?'¡Publicado!':'Publicat!'}</h2><p>${L.lang==='es'?'Tu carta de servicios ya está disponible.':'La teva carta de serveis ja està disponible.'}</p><a href="${published.url}" target="_blank" rel="noopener">${published.url}</a><button class="mobile-publish-qr-cta" type="button" data-generate-published-qr>${L.lang==='es'?'Generar QR':'Genera un QR'}<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6"/></svg></button></div>`;
        $('[data-close-mobile-publish]',box).addEventListener('click',()=>{$('#mobilePublishModal').hidden=true});
        $('[data-generate-published-qr]',box).addEventListener('click',()=>window.UncartellPlatform.openQrForPublishedUrl(published.url,state.projectName||slug));
  ...
  function openMobilePublishModal() {
    ...
    box.innerHTML = `<button class="modal-close" type="button" data-close-mobile-publish aria-label="${escapeHtml(L.close || "Tanca")}">×</button><span class="eyebrow">${escapeHtml(L.mobilePublicationTitle)}</span><h2 id="mobilePublishModalTitle">${escapeHtml(published ? L.republishMobileMenu : L.publishMobileMenu)}</h2><p>${L.lang==='es'?'Elige una dirección pública y permanente. Guardaremos los servicios y sus imágenes en tu cuenta.':'Tria una adreça pública i permanent. Desarem els serveis i les imatges al teu compte.'}</p><form class="mobile-publication-card" data-mobile-publish-form><label><span>${escapeHtml(L.businessSlug)}</span><input id="mobileSlug" type="text" maxlength="48" value="${escapeHtml(state.mobilePublication.slug || "")}" placeholder="hanoi-spa"></label><code>${L.lang==='es'?'uncartel.es/servicios':'uncartell.cat/serveis'}/<b id="mobileSlugPreview">${escapeHtml(state.mobilePublication.slug || "escriu-el-nom")}</b></code><span id="mobileSlugStatus" role="status"></span><button id="publishMobileMenu" type="submit">${escapeHtml(published ? L.republishMobileMenu : L.publishMobileMenu)}</button></form>`;
  ...
  const priceHeaderTitleMarkup = value => {
    const clean = String(value || "").replace(/\s+/g, " ").trim().slice(0, 28);
    if (clean.length <= 14) return escapeHtml(clean);
    const naturalBreak = clean.lastIndexOf(" ", 14);
    const splitAt = naturalBreak > 0 ? naturalBreak : 14;
    return `${escapeHtml(clean.slice(0, splitAt).trim())}<br>${escapeHtml(clean.slice(splitAt).trim())}`;
  };
  ...
  function blocksLayout(page, renderer = blockHtml) {
    ...
    const columnHtml = column => {
      ...
      const emptyLabel = document.documentElement.lang === "es" ? "Añade contenido" : "Afegeix contingut";
  ...
  const brandedFooter = () => {
    ...
    const logo = state.brandKit.logo ? `<img src="${state.brandKit.logo}" alt="${escapeHtml(state.brandKit.businessName || L.brandKit)}">` : "";
    const text = state.brandKit.footerText ? `<span>${escapeHtml(state.brandKit.footerText)}</span>` : "";
    return logo || text ? `<div class="brand-page-footer">${logo}${text}</div>` : "";
  };
  ...
  function wireDrag(page, menu) {
    ...
    $$("[data-empty-column]", menu).forEach(button => button.addEventListener("click", () => {
      ...
      tryInsertBlock(page, block, page.blocks.length);
    }));
    $$("[data-drag]", menu).forEach(handle => {
      ...
      handle.addEventListener("pointermove", event => {
        ...
        emptyTargetColumn = targetColumn && !target ? targetColumn : null;
  ...
  function wireInlineEditing(page, menu) {
    $$("[data-inline-field]", menu).forEach(element => {
      ...
      element.addEventListener("input", () => {
        ...
        if (!target) return;
        if (field === "priceHeaderTitle") {
          state.priceHeader.title = element.innerText.replace(/\u00a0/g, " ").replace(/\s+/g, " ").slice(0, 28);
        }
        else target[field] = element.dataset.multiline === "true" ? element.innerText : element.textContent;
        const inspectorField = $(`[data-field="${field}"]`, $("#inspector"));
        if (inspectorField && field !== "priceHeaderTitle") inspectorField.value = target[field];
        save();
      });
  ...
  function renderPage() {
    ...
    const visualHeader = $("[data-price-header]", menu);
    if (visualHeader) {
      const chooseImage = event => { if (event?.target?.closest?.("[contenteditable]")) return; $("#priceHeaderImage").click(); };
      visualHeader.addEventListener("click", chooseImage);
      visualHeader.addEventListener("keydown", event => {
  ...
  function renderProjects() {
    ...
    $$('[data-open-project]').forEach(button => button.addEventListener("click", () => {
      ...
      if (/(prova\s+de\s+foc|prueba\s+de\s+fuego)/i.test(String(projectKit.footerText || ""))) {
        projectKit.footerText = L.lang === "ca" ? "uncartell.cat" : "uncartel.es";
        projectKit.footerTextSet = false;
      }
      state = { ...state, projectId: project.id, projectName: project.name, format: project.document.format, style: project.document.style, accent: project.document.accent, textColor: project.document.text_color, contentColumns: project.document.content_columns || 1, priceHeader: project.document.price_header || { title: L.defaults.coverTitle, image: "/assets/price-header-abstract.webp?v=2", textColor: "white" }, brandKit: projectKit, pages: project.document.pages, mobilePublication: project.document.mobile_publication || { slug: "", status: "draft", publishedAt: null }, activePage: 0, selectedBlock: null, isDirty: false };
      if (state.plan === "ultra") {
  ...
  function livePricePageHtml() {
    const livePage = $("#menuPage");
    if (!livePage) return staticPageHtml(state.pages[0]);
    const clone = livePage.cloneNode(true);
    clone.removeAttribute("id");
    clone.classList.add("print-panel");
    clone.classList.remove("is-editing", "has-selection");
    clone.querySelectorAll("[id]").forEach(node => node.removeAttribute("id"));
    clone.querySelectorAll(".drag-handle,.block-context-actions,.block-context-add,.price-header-edit-hint").forEach(node => node.remove());
    clone.querySelectorAll("[contenteditable]").forEach(node => {
      node.removeAttribute("contenteditable");
      node.removeAttribute("spellcheck");
    });
    clone.querySelectorAll(".selected,.is-selected,.inline-editable:focus").forEach(node => node.classList.remove("selected", "is-selected"));
  ...
  function buildPrintDocument() {
    ...
    if (state.format === "price-a3-portrait" || state.format === "price-a3-landscape") {
      const landscape = state.format === "price-a3-landscape";
      pageSize.textContent = `@page{size:A3 ${landscape ? "landscape" : "portrait"};margin:0}`;
      root.className = `print-document print-price-list ${landscape ? "print-price-landscape" : "print-price-portrait"}`;
      root.innerHTML = `<section class="print-sheet">${livePricePageHtml()}</section>`;
    } else if (state.format === "a3-landscape") {
  ...
  $("#priceHeaderTitle").addEventListener("input", event => { const value = event.target.value.slice(0, 28); event.target.value = value; state.priceHeader.title = value; save(); renderPage(); });
  $("#priceHeaderWhite").addEventListener("click", () => { state.priceHeader.textColor = "white"; save(); renderStyles(); renderPage(); });
  $("#priceHeaderBlack").addEventListener("click", () => { state.priceHeader.textColor = "black"; save(); renderStyles(); renderPage(); });
  $("#priceHeaderRemove").addEventListener("click", () => { state.priceHeader.image = null; $("#priceHeaderImage").value = ""; save(); renderPage(); });
  $("#priceHeaderImage").addEventListener("change", event => {
    const file = event.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => { state.priceHeader.image = reader.result; save(); renderPage(); };
  ...
  $("#confirmDeleteProject").addEventListener("click", async () => {
    ...
    if (publishedSlug) {
      try { await window.UncartellPlatform?.deletePublishedDocument?.("services", publishedSlug); }
      catch (error) { console.error("Unable to remove published price list", error); toast(error.message || L.projectDeleteError || L.projectDeleted); return; }
    }
    try { await window.UncartellPlatform?.deleteUserProject?.("services", pendingDeleteProjectId); }
  ...
  let pickerTemplates = [
    { name: L.lang === "ca" ? "Plantilla genèrica" : L.lang === "it" ? "Modello generico" : "Plantilla genérica", detail: L.lang === "ca" ? "Una base neta i totalment editable." : L.lang === "it" ? "Una base pulita e completamente modificabile." : "Una base limpia y totalmente editable.", accent: "#5b9e62", blank: true }
  ];
  ...
  function renderTemplatePicker() {
    ...
    const customizeLabel=L.lang === "ca" ? "Personalitza" : L.lang === "it" ? "Personalizza" : "Personaliza";
    const customizeDesignLabel=L.lang === "ca" ? "Personalitza el disseny" : L.lang === "it" ? "Personalizza il design" : "Personaliza el diseño";
    $("#formatGrid").innerHTML = visiblePickerTemplates(selectedPickerFormat).map((template, index) => `<article class="format-card format-option${locked ? " is-premium-locked" : ""}" data-format="${selectedPickerFormat}" data-picker-card="${index}" tabindex="0" aria-label="${customizeDesignLabel} ${escapeHtml(template.name)}"><span class="format-paper-wrap"><span class="format-paper ${selectedPickerFormat}">${pickerPreview(selectedPickerFormat, index)}</span></span>${locked ? '<span class="format-plan-badge">Premium</span>' : ""}<span class="format-copy"><h2>${escapeHtml(template.name)}</h2><p>${escapeHtml(template.detail)}</p><p class="format-meta">${escapeHtml(definition?.fold || "")}</p></span><span class="format-actions"><button type="button" data-picker-template="${index}">${customizeLabel}</button></span></article>`).join("");
  ...
  $("#formatGrid").addEventListener("click", event => {
    const action = event.target.closest("[data-format-action]");
    if (!action) return;
    const format = action.closest("[data-format]")?.dataset.format;
    if (!format) return;
    if (action.dataset.formatAction === "demo") openDemo(format);
    else openEditor(format);
  });
})();
