(()=>{
  ...
  if(!document.querySelector('link[data-ui-shape-system]')){
    ...
    shapeStyles.href='/assets/ui-shape-system.css?v=20260827-ultra-pages-radius';
    shapeStyles.dataset.uiShapeSystem='true';
    document.head.append(shapeStyles);
  }
  if(!document.querySelector('link[data-catalog-tabs-system]')){
    const catalogTabsStyles=document.createElement('link');
    catalogTabsStyles.rel='stylesheet';
    catalogTabsStyles.href='/assets/catalog-tabs-system.css?v=20260828-1';
    catalogTabsStyles.dataset.catalogTabsSystem='true';
  ...
  // The local preview locale is present in the initial head. It must win on
  // the first synchronous render so brand and TLD can never be mixed.
  const previewLocale=String(window.UNCARTELL_PREVIEW_LOCALE||'').toLowerCase();
  const lang=['ca','es','it'].includes(previewLocale)?previewLocale:(i18n?.locale||'ca');
  ...
  const legacyCfg=lang==='ca'?{
    ...
    root:route(''),brand:'uncartello',tld:'it',posters:'Cartelli',postersPath:route('cartelli'),menus:'Menu e carte',menusPath:route('menu-e-carte'),prices:'Listini prezzi',pricesPath:route('listini-prezzi'),qr:'Codici QR',qrPath:route('codici-qr'),plans:'Piani',plansPath:route('piani'),account:'Account',tools:'Strumenti',about:'Chi siamo',legal:'Note legali',posterCreator:'Creatore di cartelli',menuCreator:'Creatore di menu e carte',priceCreator:'Creatore di listini prezzi',qrCreator:'Creatore di codici QR',faqs:'Domande frequenti',manifest:'Manifesto',contact:'Contatti',notice:'Note legali',privacy:'Privacy',cookies:'Cookie',settings:'Impostazioni cookie',downloads:'Download disponibili oggi',unlimited:'Download illimitati',alt:prod?'https://uncartell.cat/':'/ca/'
  };
  const market=(previewLocale&&localeConfig?.market?.(lang))||i18n?.market||{brandStem:legacyCfg.brand,tld:legacyCfg.tld,domain:`${legacyCfg.brand}.${legacyCfg.tld}`,email:lang==='ca'?'hola@uncartell.cat':'hola@uncartel.es'};
  const cfg={...legacyCfg,brand:market.brandStem,tld:market.tld,root:i18n?.urlFor('home')||legacyCfg.root,postersPath:i18n?.urlFor('posters')||legacyCfg.postersPath,menusPath:i18n?.urlFor('menus')||legacyCfg.menusPath,pricesPath:i18n?.urlFor('prices')||legacyCfg.pricesPath,qrPath:i18n?.urlFor('qr')||legacyCfg.qrPath,plansPath:i18n?.urlFor('plans')||legacyCfg.plansPath,faqsPath:i18n?.urlFor('faqs'),manifestPath:i18n?.urlFor('manifest'),contactPath:i18n?.urlFor('contact'),noticePath:i18n?.urlFor('legal'),privacyPath:i18n?.urlFor('privacy'),cookiesPath:i18n?.urlFor('cookies')};
  const publicMarketHost=window.UNCARTELL_HOSTNAME_ROUTING===true&&['uncartell.cat','uncartel.es','uncartello.it'].includes(requestedHost);
  const logoHomeHref=publicMarketHost?`https://${market.domain}/`:cfg.root;
  const authExitHref=publicMarketHost?`https://${market.domain}/`:cfg.root;
  const words=lang==='ca'?{
    ...
    language:'Lingua',languageTitle:'Scegli la lingua',loginTitle:'Accedi o registrati',loginCopy:'Accedi al tuo account per salvare i progetti e gestire il piano.',google:'Continua con Google',apple:'Continua con Apple',email:'Continua con email',name:'Il tuo nome',emailField:'La tua email',password:'Password',continue:'Accedi',createAccount:'Crea account',newHere:'Non hai ancora un account? Registrati',alreadyAccount:'Hai già un account? Accedi',forgotPassword:'Hai dimenticato la password?',resetSent:'Ti abbiamo inviato un link sicuro per reimpostarla.',resetTitle:'Crea una nuova password',newPassword:'Nuova password',savePassword:'Salva password',passwordUpdated:'Password aggiornata.',marketing:'Voglio ricevere novità e avvisi sui nuovi strumenti.',back:'Indietro',profile:'Il mio account',hello:'Ciao',plan:'Piano',manage:'Cambia piano',logout:'Esci',editName:'Modifica nome',saveName:'Salva nome',invoices:'Fatture',noInvoices:'Non ci sono ancora fatture.',backProfile:'Torna al profilo',delete:'Elimina account',deleteTitle:'Vuoi eliminare l’account?',deleteCopy:'Elimineremo l’account e tutti i dati associati. Questa azione non può essere annullata.',cancel:'Annulla',confirmDelete:'Sì, elimina account',loading:'Connessione…',confirmMail:'Controlla l’email e conferma l’account per continuare.',authError:'Non è stato possibile accedere.',invalidLogin:'Email o password non corrette.',profileSaved:'Nome aggiornato.',formSending:'Invio…',formError:'Invio non riuscito. Riprova.',cookieTitle:'Scegli tu i cookie',cookieCopy:'Utilizziamo elementi necessari al funzionamento del sito. I cookie analitici sono facoltativi e al momento non ne attiviamo nessuno.',reject:'Rifiuta facoltativi',accept:'Accetta facoltativi',configure:'Configura',necessary:'Necessari',analytics:'Analitici',save:'Salva selezione',necessaryCopy:'Sessione, sicurezza, preferenze e download.',analyticsCopy:'Misurazione aggregata dell’utilizzo. Al momento nessun fornitore attivo.'
  };
  ...
  const languageControlNode=languageTrigger?.closest('.u-language-control');
  const setLanguageMenu=(open,{focusFirst=false}={})=>{
    if(!languageTrigger||!languagePopover)return;
    if(open&&matchMedia('(min-width:981px)').matches){
      const rect=languageTrigger.getBoundingClientRect();
      document.body.append(languagePopover);
      languagePopover.style.position='fixed';
      languagePopover.style.top=`${Math.round(rect.bottom+10)}px`;
      languagePopover.style.right=`${Math.max(12,Math.round(innerWidth-rect.right))}px`;
      languagePopover.style.zIndex='1501';
    }else if(!open&&languageControlNode&&languagePopover.parentElement!==languageControlNode){
      languageControlNode.append(languagePopover);
      languagePopover.removeAttribute('style');
    }
    languagePopover.hidden=!open;
    languageTrigger.setAttribute('aria-expanded',String(open));
    document.querySelector('.u-header')?.classList.toggle('language-open',open);
  ...
  const initToolOnboarding=()=>{
    const tool=current.includes('/cartell')?'posters':current.includes('cartes-i-menus')||current.includes('cartas-y-menus')||current.includes('menu-e-carte')?'menus':current.includes('taules-de-preus')||current.includes('tablas-de-precios')||current.includes('listini-prezzi')?'services':current.includes('codis-qr')||current.includes('codigos-qr')||current.includes('codici-qr')?'qr':'';
    ...
    const sharedSlides=lang==='ca'?[{title:'Afegeix blocs',copy:'Edita el contingut arrossegant blocs des del panell o directament sobre la previsualització.',image:'/assets/tutorials/afegeix-blocs.png'},{title:'Arrossega i edita',copy:'Arrossega i edita el contingut directament des de la previsualització.',image:'/assets/tutorials/edita-des-del-visualitzador.png'},{title:'Descarrega i imprimeix',copy:'Descarrega arxius en PDF a punt per imprimir.',image:'/assets/tutorials/descarrega.png'}]:lang==='it'?[{title:'Aggiungi blocchi',copy:'Modifica il contenuto trascinando i blocchi dal pannello o direttamente nell’anteprima.',image:'/assets/tutorials/afegeix-blocs.png'},{title:'Trascina e modifica',copy:'Trascina e modifica il contenuto direttamente nell’anteprima.',image:'/assets/tutorials/edita-des-del-visualitzador.png'},{title:'Scarica e stampa',copy:'Scarica file PDF pronti per la stampa.',image:'/assets/tutorials/descarrega.png'}]:[{title:'Añade bloques',copy:'Edita el contenido arrastrando bloques desde el panel o directamente sobre la vista previa.',image:'/assets/tutorials/afegeix-blocs.png'},{title:'Arrastra y edita',copy:'Arrastra y edita el contenido directamente desde la vista previa.',image:'/assets/tutorials/edita-des-del-visualitzador.png'},{title:'Descarga e imprime',copy:'Descarga archivos en PDF listos para imprimir.',image:'/assets/tutorials/descarrega.png'}];
    const posterSlides=lang==='ca'?[{title:'Escull una icona',copy:'Fes clic damunt la icona del cartell i tria’n una o penja la teva.',image:'/assets/tutorials/icones.png'},{title:'Descarrega',copy:'Descarrega arxius en PDF a punt per imprimir.',image:'/assets/tutorials/descarrega.png'}]:lang==='it'?[{title:'Scegli un’icona',copy:'Fai clic sull’icona del cartello e scegline una oppure carica la tua.',image:'/assets/tutorials/icones.png'},{title:'Scarica',copy:'Scarica file PDF pronti per la stampa.',image:'/assets/tutorials/descarrega.png'}]:[{title:'Elige un icono',copy:'Haz clic sobre el icono del cartel y elige uno o sube el tuyo.',image:'/assets/tutorials/icones.png'},{title:'Descarga',copy:'Descarga archivos en PDF listos para imprimir.',image:'/assets/tutorials/descarrega.png'}];
    const qrSlide=lang==='ca'?{title:'Descarrega',copy:'Descarrega el teu codi QR en el format que necessitis.',image:'/assets/tutorials/descarrega.png'}:lang==='it'?{title:'Scarica',copy:'Scarica il codice QR nel formato che ti serve.',image:'/assets/tutorials/descarrega.png'}:{title:'Descarga',copy:'Descarga tu código QR en el formato que necesites.',image:'/assets/tutorials/descarrega.png'};
    const slides=tool==='posters'?posterSlides:tool==='qr'?[qrSlide]:sharedSlides;
    const doneKey=`uncartell-onboarding-${tool}-v5`;
    let step=0;
    const backdrop=document.createElement('div');backdrop.className='u-onboarding-backdrop';backdrop.hidden=true;
    const onboardingWords=lang==='ca'?{label:'Tutorial de l’eina',close:'Tanca',skip:'Omet',start:'Comença',next:'Següent'}:lang==='it'?{label:'Tutorial dello strumento',close:'Chiudi',skip:'Salta',start:'Inizia',next:'Avanti'}:{label:'Tutorial de la herramienta',close:'Cerrar',skip:'Omitir',start:'Empezar',next:'Siguiente'};
    const box=document.createElement('aside');box.className='u-tool-onboarding';box.setAttribute('role','dialog');box.setAttribute('aria-modal','true');box.setAttribute('aria-label',onboardingWords.label);box.hidden=true;
    const render=()=>{const currentSlide=slides[step],last=step===slides.length-1;box.innerHTML=`<div class="u-onboarding-media"><img src="${currentSlide.image}" alt=""><span>${step+1} / ${slides.length}</span></div><button class="u-onboarding-close" type="button" aria-label="${onboardingWords.close}">×</button><div class="u-onboarding-copy"><strong>${currentSlide.title}</strong><p>${currentSlide.copy}</p><div class="u-onboarding-dots" aria-hidden="true">${slides.map((_,index)=>`<i${index===step?' class="is-active"':''}></i>`).join('')}</div><div class="u-onboarding-actions"><button class="u-onboarding-skip" type="button">${onboardingWords.skip}</button><button class="u-onboarding-next" type="button">${last?onboardingWords.start:onboardingWords.next}</button></div></div>`;
  ...
  if(profileEditButton)profileEditButton.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';

  const upgradeWords=lang==='ca'?{
    title:'Actualitza el pla per utilitzar noves funcions',copy:'Tria el pla que encaixa amb el que necessites.',view:'Veure plans',choose:'Actualitza el pla per utilitzar noves funcions',chooseCopy:'Només l’essencial. Consulta la comparativa completa quan ho necessitis.',current:'Pla actual',recommended:'Premium · Més popular',soon:'Properament',basicCopy:'Crea, descarrega i comparteix fàcilment.',premiumCopy:'Crea, personalitza i reutilitza els teus dissenys.',ultraCopy:'Més control de marca, formats avançats i analítiques.',basicFeatures:['10 descàrregues al dia','Descàrrega en PDF','Crea codis QR','Cartells, cartes i taules de preus','Sense registre'],premiumFeatures:['Cartes per a mòbil','Descàrregues il·limitades','Desa i gestiona projectes','Descàrrega en PDF i PNG','QR dinàmics editables'],ultraFeatures:['Tot el que inclou Premium','Descàrrega en SVG','Marca d’aigua, logotip i kit de marca','Imatges i personalització avançada','Analítiques dels QR'],premiumCta:'Activa 12 mesos gratis',ultraCta:'Avisa’m',allFeatures:'Veure tots els avantatges',ultraEmail:'El teu correu',ultraSent:'T’avisarem ✓',close:'Tanca',back:'Torna enrere'
  }:lang==='it'?{
    title:'Aggiorna il piano per usare nuove funzioni',copy:'Scegli Premium o Ultra e sblocca gli strumenti che ti servono.',view:'Vedi i piani',choose:'Scegli il piano più adatto a te',chooseCopy:'Inizia con Basic o sblocca più strumenti quando ti servono.',current:'Piano attuale',recommended:'Premium · Più popolare',soon:'Prossimamente',basicCopy:'Crea e scarica senza registrarti.',premiumCopy:'Salva i progetti e personalizza i colori.',ultraCopy:'Kit del brand, logo e controllo completo della filigrana.',basicFeatures:['Crea cartelli e menu','Modifica tutti i testi','10 download al giorno','PDF pronti per la stampa'],premiumFeatures:['Tutto ciò che include Basic','Colori personalizzati','Download illimitati','Salva e recupera i progetti'],ultraFeatures:['Tutto ciò che include Premium','Aggiungi immagini','Rimuovi la filigrana','Kit del brand e logo'],premiumCta:'Attiva Premium gratis',ultraCta:'Avvisami',allFeatures:'Vedi tutti i vantaggi',ultraEmail:'La tua email',ultraSent:'Ti avviseremo ✓',close:'Chiudi',back:'Indietro'
  }:{
  ...
  const premiumAnnual=lang==='ca'?'49,99 €/any':lang==='it'?'49,99 €/anno':'49,99 €/año';
  const trialCopy=lang==='ca'?'12 mesos de prova · després, 49,99 €/any':lang==='it'?'12 mesi di prova · poi 49,99 €/anno':'12 meses de prueba · después, 49,99 €/año';
  const ultraPrice=lang==='ca'?'89,99 €/any':lang==='it'?'59,99 €/anno':'59,99 €';
  document.body.insertAdjacentHTML('beforeend',`<div class="u-upgrade-modal" data-upgrade-modal hidden><button class="u-modal-shade" type="button" data-upgrade-close aria-label="${upgradeWords.close}"></button><section class="u-upgrade-card" role="dialog" aria-modal="true" aria-labelledby="uUpgradeTitle"><button class="u-account-close" type="button" data-upgrade-close aria-label="${upgradeWords.close}"><svg viewBox="0 0 24 24">${icon('close')}</svg></button><div data-upgrade-step="intro"><h2 id="uUpgradeTitle">${upgradeWords.title}</h2><p>${upgradeWords.copy}</p><button class="u-button primary u-upgrade-view" type="button" data-upgrade-view>${upgradeWords.view}</button></div><div data-upgrade-step="plans" hidden><h2>${upgradeWords.choose}</h2><p>${upgradeWords.chooseCopy}</p><div class="u-upgrade-plans"><article data-upgrade-plan="basic"><header data-upgrade-label>${upgradeWords.current}</header><div class="u-upgrade-plan-body"><h3>Basic</h3><p>${upgradeWords.basicCopy}</p>${upgradeFeatures(upgradeWords.basicFeatures)}<div class="u-upgrade-price-stack"><strong>0 €</strong><small aria-hidden="true">&nbsp;</small></div><button type="button" data-upgrade-basic>${upgradeWords.current}</button></div></article><article class="featured" data-upgrade-plan="premium"><header data-upgrade-label>${upgradeWords.recommended}</header><div class="u-upgrade-plan-body"><h3>Premium</h3><p>${upgradeWords.premiumCopy}</p>${upgradeFeatures(upgradeWords.premiumFeatures)}<a class="u-upgrade-plan-features-link" href="${cfg.plansPath}">${upgradeWords.allFeatures||upgradeWords.view}</a><div class="u-upgrade-price-stack"><strong>0 €</strong><small class="u-upgrade-trial">${trialCopy}</small></div><button type="button" data-upgrade-premium>${upgradeWords.premiumCta}</button></div></article><article class="ultra" data-upgrade-plan="ultra"><header data-upgrade-label>${upgradeWords.soon}</header><div class="u-upgrade-plan-body"><h3>${lang==='ca'?'Premium Plus':'Ultra'}</h3><p>${upgradeWords.ultraCopy}</p>${upgradeFeatures(upgradeWords.ultraFeatures)}<a class="u-upgrade-plan-features-link" href="${cfg.plansPath}">${upgradeWords.allFeatures||upgradeWords.view}</a><div class="u-upgrade-price-stack"><strong>${ultraPrice}</strong><small aria-hidden="true">&nbsp;</small></div><button type="button" data-upgrade-ultra-notify>${upgradeWords.ultraCta}</button><div class="u-upgrade-ultra-email" data-upgrade-ultra-email hidden><input type="email" inputmode="email" autocomplete="email" placeholder="${upgradeWords.ultraEmail}" aria-label="${upgradeWords.ultraEmail}"><button type="button" aria-label="${lang==='ca'?'Envia':lang==='it'?'Invia':'Enviar'}">→</button></div><small class="u-upgrade-ultra-feedback" data-upgrade-ultra-feedback></small></div></article></div></div></section></div>`);
  const premiumPrice=document.querySelector('[data-upgrade-plan="premium"] .u-upgrade-price-stack');
  if(premiumPrice) premiumPrice.innerHTML=`<div class="u-upgrade-price-offer"><strong>0 €</strong><del>${premiumAnnual}</del></div><small class="u-upgrade-trial">${trialCopy}</small>`;
  ...
  const setupPlanAccordions=root=>{
    if(!root)return;
  ...
  document.querySelector('[data-auth-logout]').addEventListener('click',async()=>{if(supabaseClient)await supabaseClient.auth.signOut();closeAccount();location.href=authExitHref});
  ...
  const downloadFormatAccess=format=>{
    const activePlan=getEntitlementPlan(),normalized=String(format||'pdf').toLowerCase();
    if(normalized==='pdf'||normalized==='print')return {allowed:true,requiredPlan:null};
    if(normalized==='png')return {allowed:activePlan==='premium'||activePlan==='ultra',requiredPlan:'premium'};
    if(normalized==='svg')return {allowed:activePlan==='ultra',requiredPlan:'ultra'};
    return {allowed:false,requiredPlan:null};
  };
  function getEntitlementPlan(){
    if(supportContext?.plan)return ['ultra','premium'].includes(supportContext.plan)?supportContext.plan:'basic';
    if(currentProfile?.plan==='ultra'&&currentProfile.ultra_until&&new Date(currentProfile.ultra_until)>new Date())return 'ultra';
