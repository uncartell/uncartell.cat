(()=>{
  ...
  const tr=(ca,es,it)=>lang==='ca'?ca:lang==='es'?es:it;
  const T=lang==='ca'?{
  ...
  let state={id:uid(),url:'',foreground:'#181614',background:'#ffffff',level:'Q',watermark:marketDomain,logo:'',dynamicId:'',publicUrl:'',publicationSourceUrl:'',destinationUpdatedAt:'',svg:'',name:$('[data-project-name]').value};
  ...
  const encodedUrl=()=>state.publicationSourceUrl|| (state.dynamicId?(state.publicUrl||`${location.origin}/qr/?id=${state.dynamicId}`):state.url);
  ...
  const generate=async({quiet=false}={})=>{sync();if(!validUrl(state.url)){urlErrors.forEach(error=>error.textContent=T.invalid);(document.activeElement?.matches?.('[data-qr-url]')?document.activeElement:urlInputs[0]).focus();return false}urlErrors.forEach(error=>error.textContent='');try{if(canPremium&&!state.dynamicId&&!state.publicationSourceUrl)await ensureDynamicLink()}catch(error){console.error('Dynamic QR creation',error);toast(lang==='ca'?'No s’ha pogut crear el QR dinàmic':'No se ha podido crear el QR dinámico');return false}state.svg=makeSvg(encodedUrl(),state);render();$('[data-save-status]').textContent=T.generated;if(!quiet)toast(T.generated);return true};
  ...
  if(incoming&&validUrl(incoming)){
    ...
    if(incomingParams.get('source')==='publication')state.publicationSourceUrl=incoming;
  ...
  const applyEntitlements=()=>{
    ...
    const dynamicPromo=$('[data-dynamic-promo]');if(dynamicPromo){
      ...
      dynamicPromo.innerHTML=`<span><strong>${tr('QR dinàmics','QR dinámicos','QR dinamici')}</strong><small>${tr('Canvia la destinació sense tornar-los a imprimir.','Cambia el destino sin volver a imprimirlos.','Cambia la destinazione senza ristamparli.')}</small></span><em><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="10" width="14" height="10" rx="2"></rect><path d="M8 10V7a4 4 0 0 1 8 0v3"></path></svg>Premium</em>`;
    ...
    const nameField=$('[data-project-name]');nameField.disabled=false;nameField.readOnly=!canPremium;nameField.setAttribute('aria-disabled',canPremium?'false':'true');nameField.value=canPremium?(state.name||T.fallbackName):'';nameField.placeholder=canPremium?T.fallbackName:tr('Premium · Desa i gestiona projectes','Premium · Guarda y gestiona proyectos','Premium · Salva e gestisci i progetti');
