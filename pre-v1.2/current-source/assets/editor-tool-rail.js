(() => {
  if (!document.querySelector('link[href*="editor-tool-rail.css"]')) {
    ...
    styles.href = '/assets/editor-tool-rail.css?v=6-grid';
  ...
  const copy = {
    ...
    it: {
      blocks: 'Aggiungi blocchi', typography: 'Tipografia', layout: 'Colonne', header: 'Intestazione', colors: 'Colori',
      logo: 'Logo', watermark: 'Filigrana nel piè di pagina', brand: 'Kit del marchio'
    }
  };
  ...
  const installMobileSheet = (shell, panel, rail, views) => {
    ...
    bar.innerHTML = `<span aria-hidden="true"></span><button type="button" aria-label="${lang === 'ca' ? 'Tanca les eines' : lang === 'es' ? 'Cerrar herramientas' : 'Chiudi gli strumenti'}">×</button>`;
    panel.prepend(bar);
  ...
  const installContextualMobileEditing = (shell, rail, views, options = {}) => {
    ...
    const editLabel = document.documentElement.lang === 'ca' ? 'Edita el bloc' : document.documentElement.lang === 'es' ? 'Editar el bloque' : 'Modifica il blocco';
  ...
  const initialiseDocumentEditor = () => {
    ...
    rail.setAttribute('aria-label', lang === 'ca' ? 'Eines de l’editor' : lang === 'es' ? 'Herramientas del editor' : 'Strumenti dell’editor');
